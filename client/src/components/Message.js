...
export default function Message(props) {
  ...
  if (props.type === 'USER') {
    ...
    return (
      <Row>
        <Col className={`d-flex align-items-center ${state.user.name === props.user ? ' justify-content-end' : ''}`}>
          {state.user.name === props.user && date}
          <Alert className="p-1 pl-2 pr-2 mt-1 mb-1" color={state.user.name === props.user ? "danger" : "info"}>
